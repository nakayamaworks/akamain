...
| 実践起票 | 必須 | 未整理情報から自分で題名・説明・項目・証跡を組み立てる |

ゲスト利用は見本入力に限定し、実践起票、保存、AIレビュー、履歴はGoogleログインを必須とする。

### 5.3 主な画面状態

- 認証準備中、ログイン済み、未ログイン、期限切れ、エラー
...
1. Google Identity Servicesを読み込む
2. 利用者がGoogleログインする
3. GoogleからIDトークンを受け取る
4. IDトークンを`sessionStorage`とメモリに保持する
5. API呼び出し時に`Authorization: Bearer <ID token>`として送る

### 6.2 バックエンド側

1. IDトークンの署名、有効期限、発行元、対象クライアントを検証する
2. 検証済みのGoogle `sub`を内部ユーザーIDとして使用する
3. URLやリクエスト本文からユーザーIDを受け取らない
4. 本人のデータだけを検索・更新する

メールアドレスはユーザーIDに使用しない。変更可能なメールアドレスではなく、Googleが発行する不変の`sub`で識別する。

## 7. API設計

すべての`/api/*`はGoogle IDトークンを必要とする。
...
生成AIの出力は決定論的ではない。構造化出力、ルーブリック、サーバー側再計算、80件検証で揺れを抑えているが、人間の最終判断を完全に代替するものではない。
